...
environment pins the Rumoca `v0.9.13` release binary and exposes both `rumoca`
on `PATH` and `CUBS2_RUMOCA_EXECUTABLE` for CMake and tests. For non-Nix builds,
install a Rumoca CLI with the same Galec fixes and ensure `rumoca` is on `PATH`,
or set `CUBS2_RUMOCA_EXECUTABLE=/path/to/rumoca`.
